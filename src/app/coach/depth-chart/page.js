...
import { Shield, Users, ChevronDown, Save, RotateCcw } from 'lucide-react';
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {positions.map((pos, i) => (
          <motion.div key={pos.key} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
            ...
            {[1, 2, 3].map(stringNum => (
              <select key={stringNum} value={depthChart[pos.key]?.[stringNum] || ''} onChange={e => setPlayer(pos.key, stringNum, e.target.value)}
                style={{
                  padding: '6px', fontSize: 11, borderRadius: 6, cursor: 'pointer',
